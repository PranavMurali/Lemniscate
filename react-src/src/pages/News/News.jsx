...
function News() {
  ...
  return (
    <>
  <div class="ml-40 grid grid-cols-5 content-evenly self-center">
  {Data.map(article => (
    <>
    <div class=" max-w-xs h-max mx-2 overflow-hidden rounded-lg shadow-lg bg-gray-800 my-10">
        ...
        <img class="object-cover w-full h-48 mt-2" src={article.urlToImage} alt="News"/>

        <div class="flex items-center justify-between px-4 py-2 bg-gray-900">
            <h1 class="text-lg font-bold text-white">{article.publishedAt.slice(0, 10)}</h1>
            <a href={article.url} class="px-2 py-1 text-xs font-semibold text-gray-100 uppercase transition-colors duration-200 transform bg-gray-700 rounded hover:bg-gray-200 focus:bg-gray-400 focus:outline-none">Read More</a>
        </div>
    </div>
   </>
   ))}
